...
class ScoreCalculator:
    ...
    def convert_matrix(self, df_matrix, is_generated=False):
        ...
        # For OSW: convert all non-zero predicted values to neutral (999)
        # This treats any non-zero prediction (1, -1, 0.3, -0.7, etc.) as equivalent to neutral
        if is_generated:
            original_values = values_list.copy()
            values_list = [999.0 if abs(v) > 1e-10 else 0.0 for v in values_list]
            converted_count = len([v for v in original_values if abs(v) > 1e-10])
            print(f"OSW conversion: {converted_count} non-zero predictions (any value) converted to 999")

        return sources_list, targets_list, values_list

    def calculate_scores(self, gt_matrix, gen_matrix):
        self.gt_nodes_src, self.gt_nodes_tgt, self.gt_edge_dir = self.convert_matrix(gt_matrix, is_generated=False)
        self.gen_nodes_src, self.gen_nodes_tgt, self.gen_edge_dir = self.convert_matrix(gen_matrix, is_generated=True)
            ...
            for j in range(len(self.gt_edge_dir)):
                ...
                if src_score >= self.threshold and tgt_score >= self.threshold:
                    ...
                    if combined_score > best_match_score:
                        ...
                        best_match_j = j
                        # OSW NEUTRAL HANDLING:
                        # Both GT neutral (999) and predicted edges (999) should match
                        # Since we converted all non-zero predictions to 999, this becomes simple
                        if self.gt_edge_dir[j] == 999 and self.gen_edge_dir[i] == 999:
                            best_is_tp = True  # Neutral GT + any prediction = TP
                        elif self.gt_edge_dir[j] != 999 and self.gen_edge_dir[i] != 999:
                            # For non-neutral cases, check if signs match
                            gen_sign = np.sign(self.gen_edge_dir[i])
                            gt_sign = np.sign(self.gt_edge_dir[j])
                            best_is_tp = (gen_sign == gt_sign)
                        else:
                            best_is_tp = False  # Mismatch between neutral and non-neutral
        ...
        for j in range(len(self.gt_edge_dir)):
            ...
            # For neutral edges (marked as 999), any prediction counts as a match
            if best_match_score >= self.threshold or self.gt_edge_dir[j] == 999:
                gt_has_match[j] = True
...
def load_fcm_data(gt_path, gen_path, metadata_path=None):
    ...
    # SPECIAL NEUTRAL HANDLING: Convert "neutral" to special marker 999
    gt_matrix = gt_matrix.replace('Neutral', 999)
...
def main():
    parser = argparse.ArgumentParser(description='Score FCM against ground truth with OSW neutral handling')
    ...
    print("Calculating F1 score with OSW neutral handling...")
    result = scorer.calculate_scores(gt_matrix, gen_matrix)
    
    print("\n" + "="*50)
    print("FCM SCORING RESULTS (OSW NEUTRAL HANDLING)")
    print("="*50)
    ...
    print("NOTE: OSW neutral ground truth edges treated as TP for any prediction")
    print("="*50)
    
    output_file = os.path.join(output_dir, f"{data_name}_scoring_results_OSW.csv")
