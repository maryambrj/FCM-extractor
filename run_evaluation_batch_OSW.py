#!/usr/bin/env python3
"""
Batch evaluation script for FCM outputs with neutral handling.
Finds all folders and runs evaluation for each one.
Special handling: treats neutral ground truth labels as TP when prediction is 1 or -1.
"""
...
def run_evaluation(folder_name, ground_truth_dir="../ground-truth", fcm_outputs_dir="../fcm_outputs"):
    ...
    # Construct command with OSW handling flag
    cmd = [
        sys.executable,  # Use the same Python interpreter
        "utils/score_fcm_OSW.py",  # Use special OSW version
        "--gt-path", gt_path,
        "--gen-path", gen_path
    ]
    
    print(f"  📊 Evaluating {folder_name} (with OSW neutral handling)...")
...
def main():
    print("🔍 FCM Batch Evaluation Script (OSW Neutral Handling)")
    print("=" * 50)
    ...
    # Summary
    print("=" * 50)
    print("📈 EVALUATION SUMMARY (OSW Neutral Handling)")
